...
// Example of how to use the OOP component system
class DealsPage extends PageComponent {
    constructor(containerId, data = {}) {
        super(containerId, data);
        this.setupDealsPage();
    }
    ...
    setupDealsPage() {
        // Add navigation
        this.setNavigation(new NavigationComponent('nav', {
            title: 'WATCH',
            items: ['Deals', 'Discovery', 'Messages', 'Profile']
        }));

        // Add deals section
        const dealsSection = new SectionComponent('deals', {
            title: 'EXCLUSIVE DEALS',
            description: 'Discover limited-time offers from premium brands',
            layout: 'grid'
        });

        // Add deal items
        this.data.deals.forEach(deal => {
            dealsSection.addItem(new DealItemComponent('deal', deal));
        });

        this.addSection('deals', dealsSection);

        // Add footer
        this.setFooter(new FooterComponent('footer', {
            title: 'WATCH',
...
document.addEventListener('DOMContentLoaded', () => {
    const dealsPage = new DealsPage('app', {
        title: 'EXCLUSIVE DEALS',
        description: 'Discover the best luxury deals',
        deals: [
            { title: 'Luxury Watch', price: '$2,500', discount: '30% OFF' },
            { title: 'Designer Bag', price: '$1,800', discount: '25% OFF' }
        ]
    });
}); 